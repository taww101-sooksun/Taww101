...
# --- 🎭 1. ตั้งค่าดีไซน์ (Dark & Neon Orange) ---
st.set_page_config(page_title="SYNAPSE MULTI-DIMENSION", layout="centered")
...
with c2:
    st.markdown("<h3 style='margin:0; color:#FF7F50;'>SYNAPSE MULTI-DIMENSION v1.1.9</h3>", unsafe_allow_html=True)
    st.caption("สโลแกน: อยู่นิ่งๆ ไม่เจ็บตัว | มิติเสียง: ครบทุกย่าน")

# --- 🎛️ 3. แผงควบคุม "มิติลึก" (แบ่งย่านความถี่) ---
...
# --- 🚀 4. JavaScript: เครื่องยนต์เสียงสังเคราะห์ที่มีมิติลึกจริง ---
audio_engine_js = f"""
...
<script>
    ...
    btn.onclick = async () => {{
        ...
        const loop = setInterval(() => {{
            ...
            activeBar.style.background='#FF7F50'; activeBar.style.height='25px';
            ...
            // 1. ทุ้มต่ำ (Deep Bass) - Sine wave ต่ำๆ
            if(check(step, "{v_low}")) playSound(55, 'sine', 2.0, 200, false);
            // 2. แหลม (Sharp Treble) - Square wave สูงๆ สั้นๆ
            if(check(step, "{v_high}")) playSound(2500, 'square', 0.15, 4000, true);
            // 3. เสียงหลัก (Dynamic Mid) - Triangle wave มีมิติ
            if(check(step, "{v_main}")) playSound(step % 2 === 0 ? 220 : 330, 'triangle', 0.6, 1500, false);
            // 4. เอฟเฟกต์ (FX Rise) - Sawtooth wave กวาดเสียง
