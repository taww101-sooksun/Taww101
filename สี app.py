...
def room_music():
    ...
    # ฟังก์ชันเสริมสำหรับอ่านไฟล์เพลง (ต้องประกาศข้างในหรือข้างบน room_music)
    def get_base64(file_path):
        with open(file_path, "rb") as f:
            data = f.read()
        return base64.b64encode(data).decode()

    primary_neon = st.session_state.get('theme_color', "#00FFCC")
    all_songs = sorted([f for f in os.listdir('.') if f.lower().endswith('.mp3')])
    
    if not all_songs:
        st.warning("⚠️ ไม่พบไฟล์ .mp3 ในระบบ")
        return # ตอนนี้ return จะทำงานถูกต้องเพราะอยู่ในฟังก์ชัน room_music แล้ว

    # --- ส่วนของ DJ Mixer (HTML/JS) ---
    ...
    # --- ส่วนของ Simple Player ด้านล่าง ---
    st.write("---")
    current_song = all_songs[st.session_state.song_index % len(all_songs)]
    st.info(f"🎵 Simple Player: {current_song}")
