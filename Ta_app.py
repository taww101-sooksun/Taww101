...
with b3:
    if st.button("📍 จุดตัวอย่าง", use_container_width=True):
        st.session_state.points = [
            (13.75630, 100.50180),
            (13.75630, 100.50300),
            (13.75530, 100.50300),
...
if len(st.session_state.points) < 3:
    st.warning("กรุณาปักหมุดอย่างน้อย 3 จุด เพื่อคำนวณพื้นที่")
else:
    ...
    st.success(
        f"พื้นที่โดยประมาณ {rai} ไร่ {ngan} งาน "
        f"{wa} ตารางวา {remain_m2:.2f} ตร.ม."
    )
    ...
    st.markdown(
        f"""
        <div class="total-box">
            <div class="total-title">💰 ยอดรวมทั้งหมด</div>
            <div class="total-money">{money(total_cost)} บาท</div>
            <div>ไถ {money(plow_cost)} + ปั่น {money(mill_cost)}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if st.session_state.saved_plots:
    ...
    for index, item in enumerate(
        ...
        with st.expander(
            f"แปลงที่ {number} • {item['เจ้าของนา']} • "
...
st.divider()
st.caption(
    "Ta App • ระบบคำนวณพื้นที่จากพิกัด GPS เป็นค่าประมาณ "
    "ควรตรวจสอบแนวเขตจริงก่อนใช้งาน"
)
