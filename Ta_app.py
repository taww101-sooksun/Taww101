...
# สมมติชื่อผู้ใช้เพื่อแยกข้อมูล 100 คนไม่ให้มั่ว
...
if friend_gps:
    st.info(f"📍 พิกัดล่าสุดของ **{friend_name}**: Lat: {friend_gps.get('lat')}, Lon: {friend_gps.get('lon')} (เวลา: {friend_gps.get('time')})")
else:
    st.write(f"⚪ ยังไม่มีข้อมูลพิกัดของ {friend_name}")

st.markdown("---")

# 2. ปุ่มกดสั่งงานให้มือถือเปิดชิป GPS และยิงเข้า Firebase ตรงๆ (ไม่มี st.hidden_input แล้ว)
st.write("กดปุ่มด้านล่างเพื่อแชร์พิกัดปัจจุบันของคุณ:")
...
components.html(f"""
    ...
    function getLocation() {{
        ...
        if (navigator.geolocation) {{
            statusText.innerHTML = "กำลังเชื่อมต่อดาวเทียมระบุพิกัด...";
            navigator.geolocation.getCurrentPosition(sendPosition, showError, {{
                enableHighAccuracy: true, // เปิดใช้ GPS จริงจากเครื่องเพื่อความเป๊ะ
                timeout: 10000,
                maximumAge: 0
            }});
        }} else {{ 
    ...
    function sendPosition(position) {{
        ...
        document.getElementById("status").innerHTML = "จับพิกัดแม่นยำสำเร็จ! กำลังบันทึก...";
        ...
        // ยิงข้อมูลจากหน้าจอโทรศัพท์ตรงเข้า Firebase ของนายทันที ไม่ต้องผ่าน Streamlit Backend
        fetch('{FIREBASE_DB_URL}/users/{my_name}/gps.json', {{
            method: 'PUT',
            body: JSON.stringify(data)
        }}).then(response => {{
            document.getElementById("status").innerHTML = "แชร์พิกัดเข้าระบบสำเร็จแล้ว!";
        }}).catch(error => {{
    ...
    function showError(error) {{
        ...
        switch(error.code) {{
            case error.PERMISSION_DENIED:
                statusText.innerHTML = "คุณต้องกด 'อนุญาต' ให้เข้าถึงตำแหน่งก่อนนะครับ";
                break;
            case error.POSITION_UNAVAILABLE:
                statusText.innerHTML = "ไม่สามารถระบุตำแหน่งได้ (สัญญาณดาวเทียมขาดหาย)";
                break;
            case error.TIMEOUT:
                statusText.innerHTML = "หมดเวลารอสัญญาณ GPS";
                break;
            default:
                statusText.innerHTML = "เกิดข้อผิดพลาดเกี่ยวกับระบบพิกัด";
        }}
    }}
    </script>
""", height=100)
